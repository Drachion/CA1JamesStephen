...
package controllers.javascript {
  ...
  class ReverseHomeController(_prefix: => String) {
    ...
    // @LINE:14
    def employeesOnRecord: JavaScriptReverseRoute = JavaScriptReverseRoute(
      "controllers.HomeController.employeesOnRecord",
      """
        function(cat0) {
          return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "employeesOnRecord" + _qS([(cat0 == null ? null : (""" + implicitly[play.api.mvc.QueryStringBindable[Long]].javascriptUnbind + """)("cat", cat0))])})
        }
      """
    )
